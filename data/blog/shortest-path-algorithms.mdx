...
This algorithm uses the fact that in a graph with $n$ nodes, the shortest path between any two nodes cannot be longer than $n-1$ edges. So the algorithm iterates through the edges $n-1$ times and every time it checks if the previously calculated shortest paths can be improved if the include another edge.
